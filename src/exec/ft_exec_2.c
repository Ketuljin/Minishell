...
/*   Updated: 2025/03/23 07:20:36 by vdunatte         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	try_execve(t_command *command, t_env_ex *env_ex)
{
	...
	if (!handle_redir(command, true, true))
		return (1);
	...
	if (ft_strchr(args[0], '/') || get_env_var("PATH", env_ex->env) == NULL)
	{
		...
		if (stat(args[0], &buf) == 0 && S_ISDIR(buf.st_mode))
			return (print_exec_err(1, args[0], errsv), ft_tabfree(args), 126);
	}
	else
		if (!_try_execve_path(args, env_ex, &errsv))
			return (ft_tabfree(args), -1);
	if (errsv == ENOENT)
		return (print_exec_err(0, args[0], errsv), ft_tabfree(args), 127);
	return (print_exec_err(2, args[0], errsv), ft_tabfree(args), 126);
}
